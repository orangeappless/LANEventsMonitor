...
def start_firewalld_watcher(log_file, socket, threat_file, unallowed_services_list, unallowed_ports_list, threat_max, threat_mid, threat_default, passive_lower_time):
    rules_added = []
                            ...
                            if '--add-service' in command:
                                ...
                                if added_service in unallowed_services_list:
                                    ...
                                    if current_threat_level >= int(threat_max):
                                        ...
                                        # Passive lower threat generated by this action
                                        if int(passive_lower_time) > 0:
                                            passive_lower = Timer(int(passive_lower_time), threat_mgmt.update_threat, args=('clear_add_unallowed_service', threat_file))
                                            passive_lower.start()
                                            rules_added.remove(added_service)
                                    else:
                                        # Passive lower threat generated by this action
                                        if int(passive_lower_time) > 0:
                                            passive_lower = Timer(int(passive_lower_time), threat_mgmt.update_threat, args=('clear_add_unallowed_service', threat_file))
                            ...
                            # Trigger on unallowed ports
                                ...
                                if added_port in unallowed_ports_list:
                                    ...
                                    if current_threat_level >= int(threat_max):
                                        ...
                                        # Passive lower threat generated by this action
                                        if int(passive_lower_time) > 0:
                                            passive_lower = Timer(int(passive_lower_time), threat_mgmt.update_threat, args=('clear_add_unallowed_port', threat_file))
                                            passive_lower.start()
                                            rules_added.remove(added_port)
                                    else:
                                        # Passive lower threat generated by this action
                                        if int(passive_lower_time) > 0:
                                            passive_lower = Timer(int(passive_lower_time), threat_mgmt.update_threat, args=('clear_add_unallowed_port', threat_file))
