...
def start_root_watcher(audit_log, socket, block_time, threat_file, threat_max, threat_mid, threat_default, passive_lower_time):
    root_login_cmds = [
        'su',           # `sudo su`
        '/bin/bash',    # `sudo -s`
        '7375202D',     # `sudo su -`
                    ...
                    if data_attr['exe'] == "/usr/bin/sudo" and [cmd for cmd in root_login_cmds if(cmd in data_attr['cmd'])]:
                        if data_attr['res'] == 'success':
                            ...
                            if current_threat_level >= int(threat_max):
                                ...
                                # Passively lower threat generated by this action
                                passive_lower = Timer(int(passive_lower_time), threat_mgmt.update_threat, args=('clear_failed_root', threat_file))
                                passive_lower.start()
                                failed_root_attempts -= 1
                            else:
                        ...
                        if data_attr['res'] == 'success':
                            ...
                            if 'Password locked.' not in output:
                                ...
                                if current_threat_level >= int(threat_max):
                                    ...
                                    passive_lower = Timer(int(passive_lower_time), threat_mgmt.update_threat, args=('clear_failed_wheel', threat_file))
                                    passive_lower.start()
                                    failed_wheel_attempts -= 1
                                else:
                                    # Passive lower threat generated by this action
