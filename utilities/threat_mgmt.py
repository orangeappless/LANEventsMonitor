...
def create_max_threat_notif(max_threat, current_threat):
    ...
    notification = f'[{current_time}] system at, or above, MAX THREAT LEVEL of ({max_threat}), possible INCIDENT ::: [{current_threat}]'
...
def create_mid_threat_notif(mid_threat, current_threat):
    current_time = datetime.now().strftime("%Y-%m-%d %H:%M")
    notification = f'[{current_time}] system at, or above, MEDIUM THREAT LEVEL of ({mid_threat}) ::: [{current_threat}]'
